...
@section('title')
    @lang('dashboard.assessor.saldo')
@endsection

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">@lang('dashboard.assessor.asset_finance') {{ auth()->user()->name }}</h1>
        <nav class="mt-1 text-sm text-gray-500" aria-label="breadcrumb">
            <a href="/" class="hover:text-gray-700">Home</a>
            <span class="px-1">/</span>
            <span class="text-gray-700">@lang('dashboard.assessor.asset_finance')</span>
        </nav>
    </div>
    ...
    @if (auth()->user()->hasRole('asesor'))
        <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <p class="text-sm text-gray-500">Total Asset Saldo</p>
                <p class="mt-1 text-2xl font-semibold text-gray-900">Rp {{ number_format($data_finance->total) }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <p class="text-sm text-gray-500">Ditransfer / Diterima</p>
                <p class="mt-1 text-2xl font-semibold text-gray-900">Rp {{ number_format($data_finance->saldoTaken) }}</p>
            </div>
            <div class="rounded-2xl border border-gray-200 bg-white p-5">
                <p class="text-sm text-gray-500">Sisa Saldo</p>
                <p class="mt-1 text-2xl font-semibold text-gray-900">Rp {{ number_format($data_finance->total - $data_finance->saldoTaken) }}</p>
            </div>
        </div>
    @endif
    ...
    <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-gray-400">Riwayat Transfer</h2>
                ...
                <div x-data="{ open: false }" class="flex items-start justify-between gap-4 py-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-600">
                            ...
                            <button type="button" @click="open = true" class="ml-1 inline-flex items-center rounded-full bg-accent-50 px-2 py-0.5 text-[11px] font-medium uppercase text-accent-700 hover:bg-accent-100">Bukti transfer</button>
                        </p>
