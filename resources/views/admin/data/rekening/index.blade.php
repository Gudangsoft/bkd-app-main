...
@section('title')
    @lang('dashboard.rekening.title')
@endsection

@section('content')
    <div class="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">@lang('dashboard.rekening.title')</h1>
            <nav class="mt-1 text-sm text-gray-500" aria-label="breadcrumb">
                <a href="/" class="hover:text-gray-700">Home</a>
                <span class="px-1">/</span>
                <span class="text-gray-700">Data Keuangan</span>
                <span class="px-1">/</span>
                <span class="text-gray-700">@lang('dashboard.rekening.title')</span>
            </nav>
        </div>
        ...
        @if (auth()->user()->hasRole('admin') || (auth()->user()->hasRole('asesor') && !$rekening_check))
            <div x-data="{ createOpen: false }">
                <button type="button" @click="createOpen = true"
                    ...
                    <span>@lang('dashboard.rekening.create')</span>
                </button>
                ...
                <div x-show="createOpen" x-cloak class="relative z-40" role="dialog" aria-modal="true">
                    ...
                    <div class="fixed inset-y-0 right-0 flex max-w-full pl-10">
                        <div x-show="createOpen" x-transition:enter="transform transition ease-in-out duration-300"
                            ...
                            <form action="{{ auth()->user()->hasRole('admin') ? route('rekening.store') : route('rekening-setting.store') }}"
                                method="POST" enctype="multipart/form-data" class="flex h-full flex-col overflow-y-auto bg-white shadow-xl">
                                @csrf

                                <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
                                    <h3 class="text-lg font-semibold text-accent-700">@lang('dashboard.rekening.create')</h3>
                                ...
                                <div class="flex-1 space-y-4 px-6 py-4">
                                    ...
                                    @endrole
                                    <div>
                                        <label class="mb-1 block text-sm font-medium text-gray-700">Nama Nasabah</label>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="jenis_rekening"
                                            class="block w-full rounded-lg border-gray-300 text-sm focus:border-accent-500 focus:ring-accent-500">
                                    </div>
                                </div>
